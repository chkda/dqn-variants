...
from stable_baselines3.common.atari_wrappers import ClipRewardEnv, NoopResetEnv, EpisodicLifeEnv, FireResetEnv, MaxAndSkipEnv
...
def train(config):
    ...
    q_network = QNetwork(action_dims).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=config["learning_rate"])
    target_network = QNetwork(action_dims).to(device)
